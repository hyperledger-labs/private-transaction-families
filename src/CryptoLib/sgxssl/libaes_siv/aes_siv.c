...
int AES_SIV_CTX_copy(AES_SIV_CTX *dst, AES_SIV_CTX const *src) {
        safe_c_memcpy(&dst->d, sizeof dst->d, &src->d, sizeof src->d);
...
static inline int do_s2v_p(AES_SIV_CTX *ctx, block *out,
        ...
        if(len >= 16) {
                ...
                safe_c_memcpy(&t, sizeof(block), in + (len-16), 16);
                xorblock(&t, &ctx->d);
                ...
                size_t i;
                safe_c_memcpy(&t, sizeof(block), in, len);
                t.byte[len] = 0x80;
                for(i = len + 1; i < 16; i++) {
                        t.byte[i] = 0;
                }
...
int AES_SIV_DecryptFinal(AES_SIV_CTX *ctx, unsigned char *out,
        ...
        safe_c_memcpy(&q, sizeof(block), v, 16);
        q.byte[8] &= 0x7f;
        q.byte[12] &= 0x7f;
