...
static void *process_client_request(void *_thread_data)
{
	...
	try
	{
		...
		for (i = 0; i < thread_data->iterations; i++)
		{
			...
			if (packet.size >= MAX_MSG_SIZE)
			{
				PRINT(ERROR, SERVER, "packet size is too big, got %ld\n", packet.size);
				break;
			}
			...
			if (packet.size > 0)
			{
				if (send_all(thread_data->sockfd, (void *)output_data.c_str(), packet.size) == false)
				{
					PRINT(ERROR, SERVER, "send_all to %s failed\n", client_addr);
					break;
				}
...
static void close_socket(int sig, siginfo_t *siginfo, void *context)
{
	...
	PRINT(INFO, SERVER, "listening socket is closed, client reader exiting!\n");
