...
class RouteHandler:
    ...
    def __init__(
            self, loop, connection, private_tp_url,
            timeout=DEFAULT_TIMEOUT, metrics_registry=None):
        self._loop = loop
        self._connection = connection
        self._private_tp_url = private_tp_url
    ...
    async def _exchange_data_with_TP(self, data):
        '''connects to private ledger's transaction processor open socket.
            send encrypted request to TP and recieve encrypted response
        '''
        # TP socket url and port
        try:
            client_reader_url, client_reader_port = self._private_tp_url.split(":")
            client_reader_port = int(client_reader_port)
        except ValueError as e:
            print("Unable to parse private_tp_url {}: Must be in the format" +
                  " host:port".format( self._private_tp_url))
            raise

        # arguments for network struct needed to be send to TP
        network_packet_fmt = '=HHQQ'
        client_reader_type = 5        
        ...
        try:
            ...
            while True:
                try:
                    ...
                    s.close()
                    return encrypted_res.decode('ascii')
        except ConnectionRefusedError:
            s.close()
            LOGGER.error('connection to private TP failed')
            raise
        except base64.binascii.Error:
            s.close()
