...
from client_cli.workload.workload_generator import WorkloadGenerator
from client_cli.workload.sawtooth_workload import Workload
from client_cli.create_batch import create_transaction, PrivatePayload
from client_cli.create_batch import create_batch
from client_cli.exceptions import CliException
from config.config_loader import read_config_file, get_family_name, get_family_version, get_signer_key_file, get_namespace
...
class IntKeyWorkload(Workload):
    ...
    def __init__(self, delegate, args):
        ...
        if get_signer_key_file() is not None:
            try:
                with open(get_signer_key_file(), 'r') as infile:
        ...
        if key is not None:
            if key.value < 1000000:
                txn = create_transaction(
		            payload=PrivatePayload({"Verb": "inc", "Name": key.name, "Value": 1, "Prefix": "bb563a905f1957a0dd5fbf218d5d402d303ece"}, "json", signer.get_public_key().as_hex()),
    ...
    def _create_new_key(self):
        ...
        if url is not None:
            ...
            txn = create_transaction(
                payload=PrivatePayload({"Verb": "set", "Name": name, "Value": 0, "Size": 100, "Prefix": "bb563a905f1957a0dd5fbf218d5d402d303ece"}, "json", signer.get_public_key().as_hex()),
                # payload=PrivatePayload({"verb":"set", "name":name, "value":0, "Size":200}, "json", signer.get_public_key().as_hex()),
                addr= get_namespace(),
	            signer=self._signer)
            batch = create_batch(
                transactions=[txn],
